...
struct UserCutCallback : public IloCplex::UserCutCallbackI {
	...
	void main() {
		...
		double U = getBestObjValue();
		//New rule: Jun 10, 2015: if (nNodes == 0 && (mod.solveoption == 2 || mod.solveoption == 3))
...
int main(int argc, char **argv)
{
	...
	// ./ccdual-decomp instancefile eps option solveoption subproboption cutviol resultfile rootoption
	// ./ccdual-decomp instancefile 0.1 1 2 2 0.001 resultfile 0
	...
	// option = 5: same as option 4, but for CCIP
	...
	*/
	...
	if (mod.solveoption == 2 || mod.solveoption == 3)
	{
		...
		for (int j = 0; j < mod.subprob.rangelist.getSize(); ++j)
			mod.subprob.rangelist[j].end();
		mod.subprob.rangelist.end();
		mod.subprob.obj.end();
...
void buildMIP(IloEnv& env, IloModel& model, Knapsack& knap, Model& mod, int option)
{
	...
	if (mod.subproboption == 2)
	{
		cout << "do big-M strengthening..." << endl;
		...
		{	
			...
			if (mod.option != 4 && mod.option != 5)
				...
				single_sort_covering(knap, mod, i, pool);
		...
		for (int i = 0; i < knap.nbScens; ++i)
		{
			for (int k = 0; k < knap.nbDims; k++)
			{
				...
				if (mod.option != 4 && mod.option != 5)
				{
					...
					bigM = totalweight - knap.capacity[k];
				}
				else
					bigM = -knap.capacity[k];
				mod.bigM[k][i] = bigM;
			}	
		}
	}
...
void single_sort_covering(const Knapsack& knap, const Model& mod, int i, vector< vector<double> >& pool)
{
	// fixed constraint: scenario i, for all dims kk
	// solve for different obj with dim = k from 1 to dim, ii = 1 to nbScens
	for (int k = 0; k < knap.nbDims; ++k)
	{
		for (int ii = 0; ii < knap.nbScens; ++ii)
		{
			...
			if (1)
			{
				...
				sort(cost.begin(), cost.end(), greater<IndexVal>());
				double currentweight = 0.0;
				int iter = 0;
				while (currentweight > knap.capacity[kk] && iter < cost.size())
				{
					...
					{
						returnval += knap.weight[ii][k*knap.nbItems+cost[iter].ind]*(currentweight-knap.capacity[kk])*1.0/knap.weight[i][kk*knap.nbItems+cost[iter].ind];
						break;
					}
					iter++;
				}
